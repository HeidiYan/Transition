...
# 1. read raw data for experiment 1
...
## build one model for single subject
def model_construct(sub_id, model_type, sp):
    filepath = 'experiment1'
...
## get model waic for all models of a single subject
...
if __name__ == '__main__':
    ...
    tpdata=pd.read_csv('exp1_tp.csv')
    ...
    ##
    # import data
    mydata=pd.read_csv('exp1_expdata.csv')
    # delete if acc==0
    mydata.dropna(axis=0,how='any', inplace=True)
    print(mydata)

    ##
    ## export waic for all subject in exp1
